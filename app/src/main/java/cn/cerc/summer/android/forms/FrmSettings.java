...
//            lin_cun.setVisibility(View.GONE);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.button1:
                scales = ScreenUtils.getScales(4);
                break;
            case R.id.button2:
                scales = ScreenUtils.getScales(4.5);
                break;
            case R.id.button3:
                scales = ScreenUtils.getScales(5);
                break;
            case R.id.button4:
                scales = ScreenUtils.getScales( 5.5);
                break;
            case R.id.button5:
                scales = ScreenUtils.getScales( 6);
                break;
        }
        customseekbar.setProgress(scales);
        String str = String.format("界面缩放比例（80%% -- 120%%）当前值：%d%%", scales);
